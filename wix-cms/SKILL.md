---
name: wix-cms
description: Client-only Wix CMS (Wix Data) scaffold (dependency-free REST). Use when building or extending a site over an existing Wix data collection — listing content, item detail pages, filtering/search, public form submissions, and full data-item CRUD. The public WIX_CLIENT_ID is provided in the handoff prompt and hardcoded into the snippet (it is buyer-facing, not a secret). Works as an anonymous visitor, so every call is bound by the collection's per-role permissions (Read/Insert for "Anyone"). The site owner creates collections and sets permissions in the Wix dashboard. Never provisions collections, never mocks data.
---
