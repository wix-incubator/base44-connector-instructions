...
description: Client-only Wix Pricing Plans scaffold (dependency-free REST). Use when building or extending a Wix-powered "Plans & Pricing" / membership experience over an existing Wix site that already has pricing plans — wiring live plan fetching (subscriptions, single-payment, free plans, free trials), a plan detail page, member-only purchase via Wix-hosted checkout (login + payment), and a "my plans" screen of the member's orders. The public WIX_CLIENT_ID is provided in the handoff prompt and hardcoded into the snippet (it is buyer-facing, not a secret). Read-only over plans; the merchant creates plans in the Wix dashboard. Never provisions plans, never mocks them, never hand-builds checkout URLs.
---
