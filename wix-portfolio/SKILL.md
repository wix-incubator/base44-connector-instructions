---
name: wix-portfolio
description: Client-only Wix Portfolio showcase scaffold (dependency-free REST). Use when building or extending a Wix-powered portfolio over an existing Wix site with Wix Portfolio installed — wiring live collections, projects, and per-project media galleries (images/videos). The public WIX_CLIENT_ID is provided in the handoff prompt and hardcoded into the snippet (it is visitor-facing, not a secret). Read-only over existing content; the owner adds collections, projects, and items in the Wix dashboard. Never provisions, never mocks projects.
---
...
2. The site's public headless **`WIX_CLIENT_ID`**, provided in the handoff prompt (the Wix
   Business Manager surfaces a copyable prompt with the id filled in — see `PROMPT.md`).
