---
name: wix-bookings
description: Client-only Wix Bookings scaffold (dependency-free REST). Use when building or extending a Wix-powered appointment booking experience over an existing Wix Bookings site — listing services, showing available appointment time slots, creating a booking, and completing it through redirect-session hosted checkout. The public WIX_CLIENT_ID is provided in the handoff prompt and hardcoded into the snippet (it is buyer-facing, not a secret). Read-only over services; the owner creates services in the Wix dashboard. Never provisions, never mocks services.
---
