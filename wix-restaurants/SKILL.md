...
description: Client-only Wix Restaurants scaffold (dependency-free REST). Use when building or extending a Wix-powered restaurant site over an existing Wix Restaurants setup — wiring a live menu (menus → sections → items, with price variants, modifier groups, and labels), online ordering through the Wix-owned eCom cart with redirect-session checkout, and table reservations (locations → time slots → hold → reserve). The public WIX_CLIENT_ID is provided in the handoff prompt and hardcoded into the snippet (it is buyer-facing, not a secret). Read-only over menu content; the restaurant manages menus, operations, and locations in the Wix dashboard. Never provisions, never mocks menu data.
---
...
3. The site's public headless **`WIX_CLIENT_ID`**, provided in the handoff prompt (see `PROMPT.md`).
